...
function Display({name,data,id}:{name:string,data:string,id:string}){
        ...
        return(
            <div className="w-full cursor-pointer" onClick={toggle}>{data}</div>
        )
}
export default function DataItem({result}:{result:UScore}){
    ...
    return (
        <div className="flex gap-x-8 w-[700px] border border-black rounded-md justify-start items-center">
            ...
            <div className="w-[35px]  text-center">{result.total}</div>
            <div className="w-[35px]  text-center">{result.grade}</div>
